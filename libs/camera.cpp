...
void camera::draw_vector(float *v1, float *v2)
{
       ...
       bool res1 = cord_from_vector(v1, cord1);
       bool res2 = cord_from_vector(v2, cord2);
       if(res1 && res2){
...
void camera::draw_dot(float *v1, int size)
{
       BYTE clr[4] = {0x00, 0x00, 0xFF, 0xFF};
       float cord1[3] = {0,0,0};
       bool res = cord_from_vector(v1, cord1);
       if(res){
       paint_cord(cord1, size, clr);
       }
}
...
void camera::draw_medium(medium m, float *p)
{
       float pos[3] = {0,0,0};
       float prev[3] = {0,0,0};
       ...
       for(int x = 0; x < m.width; x++)
       {
              ...
              {
                     m.copy_node_pos(x, y, actual);
                     addVec(actual, pos, actual);
                     draw_dot(actual, 2);

                     if(y != 0){
                            m.copy_node_pos(x, y-1, prev);
                            addVec(prev, pos, prev);
                            prev[1] -= m.get_ly();

                            draw_vector(actual, prev);
                     }
                     if(x != 0){
                            m.copy_node_pos(x-1, y, prev);
                            addVec(prev, pos, prev);
                            prev[0] -= m.get_lx();
...
void camera::screen_cords(float *w, float *depth, float *cords)
{
       ...
       if(dotProduct(yComp, yaxis) < 0){cords[1] = cords[1] * -1;}
       ...
       if(dotProduct(xComp, xaxis) < 0){cords[0] = cords[0] * -1;}
              ...
              for(int dy = -1*size; dy <= size; dy++){
                     ...
                     if(in_range(temp, height, width))
                     {
                            ...
                            if(depthScreen[n] >= cord[2]){
                                   ...
                                   screen[n*4] = clr[0];// blue
                                   screen[n*4+1] = clr[1]; //green
                                   screen[n*4+2] = clr[2];
